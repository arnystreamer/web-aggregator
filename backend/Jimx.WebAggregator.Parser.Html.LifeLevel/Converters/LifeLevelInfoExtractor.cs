...
public class LifeLevelInfoExtractor
{
     public CityDataItem[] Extract(string html, string dataSetName)
     {
          ...
          var newDataItems = dataSet.Data
               .Select(row => new CityDataItem(
                    $"{dataSetName} - {row.Subsection} - {row.Data[0]}",
                    row.Data[1].ParseToDecimal(),
                    row.Data[2]?.ParseToDecimalRangeDashedLeft() ?? null,
                    row.Data[2]?.ParseToDecimalRangeDashedRight() ?? null))
               .ToArray();
