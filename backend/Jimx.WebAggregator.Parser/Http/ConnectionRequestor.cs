...
public class ConnectionRequestor : IDisposable
{
	...
	public async Task<HttpResponseMessage> EnqueueRequestAsync(Uri uri, HttpMethod method, HttpHeaders? overrideHeaders)
	{
		...
		var headers = overrideHeaders != null ? _defaultHeaders.Union(overrideHeaders) : _defaultHeaders;
		...
		_logger.LogInformation("ConnectionRequestor (count={QueueItemsCount}): Request enqueued {S}", 
			_queueItems.Count, uri.ToString());
